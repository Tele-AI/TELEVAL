...
class AudioLLM(torch.nn.Module):
    ...
    def recognize(
        ...
        do_sample = extra_inputs.get('do_sample', True)
    ...
    def _post_decode(self, output, temperature=1.0, top_k=0, top_p=0.0):
        ...
        token_index = torch.multinomial(probs, 1)  # sampling randomly
    ...
    def _generate_one_step(
        ...
        if stat == 'sl' or stat == 'cl':
            ...
            prob = F.softmax(state_logits[:, :-1], dim=-1)
        ...
        if do_sample:
            ...
            last_id = torch.argmax(last_logit.squeeze(0).squeeze(0), dim=-1, keepdim=True).unsqueeze(0)
