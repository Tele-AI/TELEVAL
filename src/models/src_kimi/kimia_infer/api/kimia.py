...
from src.models.src_kimi.kimia_infer.models.detokenizer import get_audio_detokenizer
from .prompt_manager import KimiAPromptManager
from src.models.src_kimi.kimia_infer.utils.sampler import KimiASampler
from src.models.model_utils import load_model_with_auto_device_map
...
class KimiAudio(object):
    def __init__(self, model_path: str, whisper_path: str, glm4_tokenizer: str, load_detokenizer=True, split_device=False):
        ...
        if split_device:
            self.alm = load_model_with_auto_device_map(
                model_name=cache_path,
                no_split_module_classes=["MoonshotDecoderLayer"],
                dtype=torch.bfloat16
            )
            logger.info(f"self.alm slice: {self.alm.hf_device_map}")
        else:
            self.alm = AutoModelForCausalLM.from_pretrained(
                cache_path, torch_dtype=torch.bfloat16, trust_remote_code=True, device_map='auto'
            )
        ...
        self.prompt_manager = KimiAPromptManager(
            model_path=cache_path, whisper_path=whisper_path, glm4_tokenizer=glm4_tokenizer, kimia_token_offset=self.kimia_token_offset
        )
        # if no need of tts, load_detokenizer=False may release 12GB GPU memory cost
        if load_detokenizer:
